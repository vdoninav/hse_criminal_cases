...
if params.RENEW_SAVED_DATA_IN_PREPROCESSING:
    ...
    RuLegalNER_train['entities'] = RuLegalNER_train['entities'].apply(
        lambda entities: [dict(zip(params.KEYS, values + [params.TYPES[values[3]]])) for values in entities])
    RuLegalNER_test['entities'] = RuLegalNER_test['entities'].apply(
        lambda entities: [dict(zip(params.KEYS, values + [params.TYPES[values[3]]])) for values in entities])
    RuLegalNER_validation['entities'] = RuLegalNER_validation['entities'].apply(
        lambda entities: [dict(zip(params.KEYS, values + [params.TYPES[values[3]]])) for values in entities])

    """ Tokenizing datasets + saving them """
    ner_train = pd.DataFrame([prepare_data.extract_labels(RuLegalNER_train.loc[i]) for i in
                              tqdm(range(RuLegalNER_train.shape[0]), desc='Extracting train labels')])
    ner_train.to_pickle(params.SAVE_DIR + "tokenized_train.pkl")
    del RuLegalNER_train
    print('saved')

    ner_test = pd.DataFrame([prepare_data.extract_labels(RuLegalNER_test.loc[i]) for i in
                             tqdm(range(RuLegalNER_test.shape[0]), desc='Extracting test labels')])
    ner_test.to_pickle(params.SAVE_DIR + "tokenized_test.pkl")
    del RuLegalNER_test
    print('saved')

    ner_validation = pd.DataFrame([prepare_data.extract_labels(RuLegalNER_validation.loc[i]) for i in
                                   tqdm(range(RuLegalNER_validation.shape[0]), desc='Extracting valid labels')])
